...
export function AnnouncementCard({ announcement }: AnnouncementCardProps) {
  const club = getClubById(announcement.clubId);
  return (
    <Card className="transition duration-200 hover:-translate-y-1 hover:shadow-lift">
      <CardContent>
        <div className="flex items-start gap-3">
          <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-[12px] ${announcement.priority === "important" ? "bg-amber-50 text-amber-600" : "bg-brand-mist text-brand-forest"}`}>
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-500">{club?.name}</span>
            </div>
