    ...
    <div className="min-h-screen bg-white text-brand-ink">
      <header
        className={cn(
          ...
          scrolled ? "border-brand-mist bg-white/92 shadow-sm" : "border-transparent bg-white/82",
        )}
      >
        <div className="mx-auto flex h-[72px] w-full max-w-[1180px] items-center justify-between px-5 md:px-6">
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.href || pathname.startsWith(`${item.href}/`) ? "text-brand-forest" : "text-brand-ink",
                )}
              >
                {item.label}
              </Link>
          ...
          <div className="flex items-center gap-1.5">
            <Link href="/login" className="hidden rounded-lg px-3.5 py-3 text-[15px] font-semibold transition hover:text-brand-forest sm:inline-flex">
              Sign In
            </Link>
            <Link
              href="/register"
        ...
        {menuOpen ? (
          <div className="border-t border-brand-mist bg-white py-3 md:hidden">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="block px-6 py-3 text-base font-medium hover:bg-brand-surface hover:text-brand-forest" onClick={() => setMenuOpen(false)}>
                {item.label}
              </Link>
            ))}
            <div className="grid gap-2 px-6 pb-2 pt-3">
              <Link href="/login" className="inline-flex items-center justify-center rounded-[11px] border border-brand-mist bg-white px-5 py-3 text-[15px] font-semibold text-brand-forest transition hover:bg-brand-surface" onClick={() => setMenuOpen(false)}>
                Sign In
              </Link>
              <Link href="/register" className="inline-flex items-center justify-center rounded-[11px] bg-brand-forest px-5 py-3 text-[15px] font-semibold text-white transition hover:bg-brand-forestDark" onClick={() => setMenuOpen(false)}>
                Create Account
              </Link>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          <div className="max-w-xs">
            ...
            <p className="mt-3 text-sm leading-6 text-brand-muted">
              Discover clubs, events, and student communities at Farmingdale State College.
            </p>
          </div>
          <div className="grid gap-8 sm:grid-cols-3">
            <FooterColumn title="Explore" links={[["Clubs", "/clubs"], ["Events", "/events"], ["About", "/about"]]} />
            <FooterColumn title="Account" links={[["Sign In", "/login"], ["Create Account", "/register"], ["Help", "/about"]]} />
            <FooterColumn title="College" links={[["Farmingdale", "https://www.farmingdale.edu"], ["Student Life", "/about"], ["Contact", "/about"]]} />
          </div>
        </div>
  ...
  return (
    <div>
      ...
      {links.map(([label, href]) => (
        <Link key={`${title}-${label}`} href={href} className="block py-1 text-sm text-brand-muted transition hover:text-brand-forest">
          {label}
        </Link>
      ))}
    </div>
