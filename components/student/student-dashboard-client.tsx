...
import type { Announcement, Club, EventItem, JoinRequest, NotificationItem as NotificationType, StudentProfile } from "@/lib/types";
...
export function StudentDashboardClient({
  ...
  useEffect(() => {
    ...
    async function loadRequests() {
      if (!user) {
        setStudentRequests(joinRequests.filter((request) => request.studentId === fallbackStudent.id));
        return;
      }
      try {
        const requests = await getStudentJoinRequests(user.uid);
        if (active) {
  ...
  }, [fallbackStudent.id, joinRequests, user]);
  ...
  const requestsByClub = new Map(studentRequests.map((request) => [request.clubId, request]));

  const suggestedClubs = clubs
    .filter((club) => club.isSuggested && !student.joinedClubIds.includes(club.id))
    .slice(0, 3)
    .map((club) => ({
      ...club,
      isSaved: savedClubIds.has(club.id),
      membershipStatus: requestsByClub.get(club.id)?.status ?? club.membershipStatus ?? "notJoined",
    }));
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Saved clubs" value={student.savedClubIds.length} detail="Bookmarked communities" icon={<Users className="h-5 w-5" />} color="gold" />
        ...
        <div className="space-y-4">
          ...
          {studentRequests.length ? (
            studentRequests.map((request) => {
              const club = clubs.find((clubItem) => clubItem.id === request.clubId);
              return (
                <Card key={request.id}>
                  <CardContent>
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-brand-muted">Membership requests will appear here after you ask to join a club.</p>
              </CardContent>
            </Card>
          )}
        </div>
