...
import { announcements, clubs, currentStudent, events, getClubById, joinRequests, notifications } from "@/lib/mock-data";
import type { Club, EventItem, JoinRequest } from "@/lib/types";
import { type FirebaseStudentProfile, getStudentProfile } from "@/lib/firebase/user-profile";
import { getStudentJoinRequests } from "@/lib/firebase/student-actions";
...
const fallbackRequests = joinRequests.filter((request) => request.studentId === currentStudent.id);

export function StudentDashboardClient() {
  const { user } = useAuth();
  const [profile, setProfile] = useState<FirebaseStudentProfile>(fallbackProfile);
  const [studentRequests, setStudentRequests] = useState<JoinRequest[]>(fallbackRequests);
  ...
  useEffect(() => {
    if (!user) {
      ...
      setStudentRequests(fallbackRequests);
      return;
    }

    let isActive = true;
    setStatusMessage("Loading personalized dashboard data...");

    Promise.all([getStudentProfile(user), getStudentJoinRequests(user.uid)])
      .then(([studentProfile, requests]) => {
        if (isActive) {
      ...
      .catch(() => {
        if (isActive) {
          setStatusMessage("Unable to load all personalized dashboard data right now.");
        }
      });
  ...
  const personalizedClubs = useMemo<Club[]>(() => {
    return clubs.map((club) => {
      const matchingRequest = studentRequests.find((request) => request.clubId === club.id);
      const isJoined = profile.joinedClubIds.includes(club.id);
      return {
        ...club,
        isSaved: profile.savedClubIds.includes(club.id),
        membershipStatus: isJoined ? "approved" : matchingRequest?.status ?? "notJoined",
      };
    });
  }, [profile.joinedClubIds, profile.savedClubIds, studentRequests]);

  const personalizedEvents = useMemo<EventItem[]>(() => {
    return events.map((event) => ({
      ...event,
      isSaved: profile.savedEventIds.includes(event.id),
      hasRsvped: profile.rsvpedEventIds.includes(event.id),
    }));
  }, [profile.rsvpedEventIds, profile.savedEventIds]);

  const joinedClubs = personalizedClubs.filter((club) => club.membershipStatus === "approved");
  const suggestedClubs = personalizedClubs
    .filter((club) => club.isSuggested && !profile.joinedClubIds.includes(club.id))
    .slice(0, 3);

  const upcomingEvents = personalizedEvents.slice(0, 3);
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        title={`Welcome back, ${profile.displayName.split(" ")[0] || "student"}!`}
        description="Your home base for club updates, upcoming events, join requests, and campus notifications."
        action={
          <Link
            href="/clubs"
            className="inline-flex h-11 items-center justify-center gap-2 rounded-[11px] bg-brand-forest px-4 text-sm font-semibold leading-none text-white shadow-[0_6px_16px_rgba(11,93,59,0.18)] transition hover:bg-brand-forest/90"
          >
      ...
      {statusMessage ? (
        <Card>
          <CardContent>
            <p className="text-sm font-semibold text-brand-ink">{statusMessage}</p>
          </CardContent>
        </Card>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Saved clubs" value={profile.savedClubIds.length} detail="Bookmarked communities" icon={<Users className="h-5 w-5" />} color="gold" />
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_0.85fr]">
        ...
        <div className="space-y-4">
          ...
          {studentRequests.length ? (
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-brand-muted">No join requests yet.</p>
              </CardContent>
            </Card>
          )}
        </div>
