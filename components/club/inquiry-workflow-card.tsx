...
export function InquiryWorkflowCard({ inquiry }: InquiryWorkflowCardProps) {
  ...
  async function sendReply() {
    ...
    setFeedback("");
    try {
      const reply = await replyToInquiry(inquiry.id, trimmedReply);
      setReplies((current) => [...current, reply]);
      setReplyText("");
      setReplyVisible(true);
      setFeedback("Reply saved to Firestore.");
    } catch {
  ...
  async function markResolved() {
    ...
    try {
      ...
      setStatus("resolved");
      setFeedback("Inquiry marked resolved.");
    } catch {
      setFeedback("Unable to resolve inquiry right now.");
    } finally {
      setResolving(false);
    }
  }
      ...
      <CardContent>
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-brand-muted">
                {inquiry.studentName ?? "Student"} to {inquiry.clubName ?? "Club"}
              </p>
              ...
              {feedback ? <p className="mt-3 text-sm font-medium text-brand-forest">{feedback}</p> : null}
            </div>
          </div>
          <div className="flex flex-wrap gap-2 md:justify-end">
            ...
            <Button size="sm" variant="secondary" disabled={status === "resolved" || resolving} onClick={markResolved}>
        ...
        {replyVisible ? (
          <div className="mt-5 rounded-[14px] border border-brand-mist bg-white p-4">
            ...
            <Button className="mt-3" size="sm" onClick={sendReply} disabled={sending || !replyText.trim()}>
