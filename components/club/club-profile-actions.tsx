...
export function ClubProfileActions({ club }: ClubProfileActionsProps) {
  ...
  const [joinMessage, setJoinMessage] = useState(`I would like to join ${club.name}.`);
  ...
  const profileSaved = profile?.savedClubIds.includes(club.id) ?? club.isSaved ?? false;
  ...
  return (
    <div className="rounded-[18px] border border-brand-mist bg-white p-5 shadow-[0_1px_2px_rgba(7,61,39,0.04),0_10px_28px_rgba(7,61,39,0.06)]">
      ...
      <label className="mt-5 block">
        <span className="text-sm font-semibold text-brand-ink">Join request note</span>
        <textarea
          className="mt-2 min-h-24 w-full rounded-[12px] border border-brand-mist bg-white px-3.5 py-3 text-sm text-brand-ink outline-none transition placeholder:text-brand-muted/70 focus:border-brand-green focus:ring-2 focus:ring-brand-green/15"
          value={joinMessage}
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row">
        ...
        >
          ...
          {isJoining ? "Sending..." : status === "approved" ? "Already joined" : status === "pending" ? "Request pending" : "Request to join"}
        </Button>
      ...
      {messageVisible ? (
        <form className="mt-5 rounded-[14px] bg-brand-mist p-4" onSubmit={handleQuestionSubmit}>
          <p className="text-sm font-semibold text-brand-forest">Official club inquiry</p>
          <p className="mt-1 text-sm leading-6 text-brand-muted">
            This sends a question to the official club inbox, not a student-to-student direct message.
          </p>
          <textarea
            className="mt-3 min-h-24 w-full rounded-[12px] border border-brand-mist bg-white px-3.5 py-3 text-sm text-brand-ink outline-none transition placeholder:text-brand-muted/70 focus:border-brand-green focus:ring-2 focus:ring-brand-green/15"
            value={question}
            onChange={(event) => setQuestion(event.target.value)}
            placeholder="Write your question for the club officers."
            required
          />
          <Button className="mt-3" size="sm" type="submit" disabled={isSendingQuestion || !question.trim()}>
