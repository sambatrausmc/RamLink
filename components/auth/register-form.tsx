...
export function RegisterForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Success! Kick them to the profile page
      router.push("/profile");
    } catch {
  ...
  return (
    <form className="mt-7 space-y-4" onSubmit={handleSubmit}>
      <label className="block">
        ...
        <Input className="mt-2" value={displayName} onChange={(event) => setDisplayName(event.target.value)} required />
      </label>
      <label className="block">
      ...
      <label className="block">
        ...
        <Input className="mt-2" value={password} onChange={(event) => setPassword(event.target.value)} type="password" minLength={6} required />
      </label>
