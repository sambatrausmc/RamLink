...
export default async function ClubProfilePage({ params }: ClubProfilePageProps) {
    ...
    // fetch all the related content for this specific club
    const clubEvents = getEventsForClub(club.id);
    const clubAnnouncements = getAnnouncementsForClub(club.id);
    const clubResources = getResourcesForClub(club.id);
        ...
        <div className="bg-brand-surface/70">
            <div className="mx-auto w-full max-w-[1180px] space-y-10 px-5 py-12 md:px-6 md:py-16">
                ...
                <PageHero
                    ...
                    actions={
                        <>
                            {/* prompt unregistered users to log in if they want to join */}
                            <Link
                                href="/login"
                    ...
                    aside={
                        // this side card displays all the quick logistical info for the club
                        <div className="rounded-[22px] border border-brand-mist bg-white p-5 shadow-lift">
                            <div className="grid h-16 w-16 place-items-center rounded-[18px] bg-brand-mist font-display text-xl font-semibold text-brand-forest">
                                {club.shortName}
                            </div>
